...
namespace :deploy do
  ...
  desc "Restart dev nextjs node server"
  task :nextjs_restart_node_dev do
    on roles(:web) do
      execute :salt, '-N', "node-dev", 'service.restart', "node-dev-cdhdb"
    end
  end

  desc "Restart stage nextjs node server"
  task :nextjs_restart_node_stage do
    on roles(:web) do
      execute :salt, '-N', "node-dev", 'service.restart', "node-stage-cdhdb"
    end
  end
end
